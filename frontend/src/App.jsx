...
export default function App() {
  const [mode, setMode] = useState("solo"); // "solo" | "compare" | "watches"
  ...
  function pushLocalHistory(entry) {
    setLocalHistory((h) => [{ id: crypto.randomUUID(), ...entry }, ...h].slice(0, 25));
  }
  ...
  async function runScan(e) {
    ...
    try {
      if (mode === "solo") {
        ...
        const data = await analyzeTopic(topic, limit, platforms);
        setResult(data);
        if (!user) {
          pushLocalHistory({
            mode: "solo",
        ...
        setResult(null);
        const data = await compareTopics(topic, topicB, limit, platforms);
  ...
  function selectLocalHistory(entry) {
  ...
  function selectRemoteHistory(row) {
    // Saved scans store the full result payload for the primary topic.
    // (Compare-mode history currently restores topic A's data only.)
    setMode("solo");
    ...
    <div className="shell">
      <header className="masthead">
        <div className="masthead-title">
          <span className="masthead-mark">TR-02</span>
          <div>
            <h1>TubeRadar</h1>
      ...
      <div className="mode-switch">
        ...
        <button className={mode === "watches" ? "active" : ""} onClick={() => setMode("watches")}>
          Watches
        </button>
      </div>
          ...
          {mode === "solo" ? (
            ...
            <div className="field-row">
              <div className="field">
                ...
                <input id="topicA" type="text" value={topic} onChange={(e) => setTopic(e.target.value)} required />
              </div>
              <div className="field">
                <label htmlFor="topicB">Contender B</label>
                <input id="topicB" type="text" value={topicB} onChange={(e) => setTopicB(e.target.value)} required />
              </div>
            </div>
          )}
      ...
      {mode === "watches" && <WatchesPanel user={user} defaultEmail={user?.email} />}

      {mode !== "watches" && !loading && !error && !result && !compareResult && (
      ...
      <HistoryDrawer
        ...
        onSelectLocal={selectLocalHistory}
        onSelectRemote={selectRemoteHistory}
      />
    </div>
  );
}
